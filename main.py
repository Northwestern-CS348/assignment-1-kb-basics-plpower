...
class KBTest(unittest.TestCase):
    ...
    def test6(self):
        ask1 = read.parse_input("fact: (isa dog monster)")
        print(' Asking if', ask1)
        answer = self.KB.kb_ask(ask1)
        self.assertFalse(answer)
    ...
    def test8(self):
        ask1 = read.parse_input("fact: (size ?X big)")
        print(' Asking if', ask1)
        answer = self.KB.kb_ask(ask1)
        self.assertEqual(str(answer[0]), "?X : bigbox")
        self.assertEqual(str(answer[1]), "?X : pyramid3")
    ...
    def test9(self):
        ask1 = read.parse_input("fact: (size bigbox ?X)")
        print(' Asking if', ask1)
        answer = self.KB.kb_ask(ask1)
        self.assertEqual(str(answer[0]), "?X : big")
